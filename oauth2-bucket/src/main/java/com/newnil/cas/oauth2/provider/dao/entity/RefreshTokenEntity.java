...
public class RefreshTokenEntity extends AbstractAuditable<Long> {
    ...
    @NonNull
    @NotNull
    @Column(name = "token_id", nullable = false, unique = true, length=10000)
    private String tokenId;

    @NonNull
    @NotNull
    @Convert(converter = OAuth2RefreshTokenPersistenceConverters.class)
    @Column(name = "serialized_token", nullable = false,length=10000)
    ...
    @OneToMany(mappedBy = "refreshToken", fetch = FetchType.LAZY,cascade = CascadeType.ALL)
